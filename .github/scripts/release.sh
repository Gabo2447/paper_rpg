...
printHelp() {
  ...
  echo '    Bump'
  echo '        Bump the version of lazy versioned modules to the current version of SoulBound.'
...
versionCurrent() {
  ...
  if [ "$(git tag -l "v$CURRENT_VERSION")" ]; then
    echo "    soulbound: $CURRENT_VERSION (already released)"
  else
    echo "    soulbound: $CURRENT_VERSION"
  fi
}
...
releasePublish() {
  ...
  echo '    Creating version tag for soulbound...'
...
#  local url="https://raw.githubusercontent.com/Gabo2447/SoulBound/refs/heads/gh-pages/$major/Documentation/CHANGELOG/index.html"
#
#  local response
...
setupPrepareSelectTimeDefaultValue() {
  ...
  set +e
  GH_RELEASE_JSON="$(gh release view "v$CURRENT_VERSION" --repo "Gabo2447/SoulBound" --json publishedAt >&1 2> /dev/null)"
  set -e
...
setupCommit() {
  ...
  echo '    Updating SoulBound pom.xml file...'
  ...
  for module in $(./mvnw -B -DforceStdout help:evaluate -Dexpression=project.modules | sed -n 's:.*<string>\(.*\)</string>.*:\1:p'); do
    ...
    find . -name "pom.xml" -type f -exec sed -i \
      "s|<soulbound\.${module}\.version>[^<]*</soulbound\.${module}\.version>|<soulbound.${module}.version>${NEW_VERSION}\${changelist}</soulbound.${module}.version>|g" {} +
  done
  ...
  git -c core.safecrlf=false commit --all --message="Bump version of SoulBound to $NEW_VERSION" 2>&1 > /dev/null | sed 's/^/        /'
}

bumpCommit() {
  echo 'Bump'

  echo "    Updating pom.xml files for modules $BUMP_MODULES..."
...
bumpPrepareSelectTimeDefaultValue() {
  ...
  set +e
  GH_RELEASE_JSON="$(gh release view "v$LAST_API_LIB_VERSION" --repo "Gabo2447/SoulBound" --json publishedAt >&1 2> /dev/null)"
  set -e
