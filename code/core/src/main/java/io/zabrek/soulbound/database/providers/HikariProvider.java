package io.zabrek.soulbound.database.providers;
...
public class HikariProvider implements ConnectionProvider {
     ...
     */
    public HikariProvider(final Logger log, final HikariDriver driver, final String... args) {
        this.log = log;
        this.driver = driver;
        ...
        log.info("Initializing HikariCP pool for driver '%s'".formatted(driver.name()));
        final HikariConfig config = driver.getConfig(args);

        this.dataSource = new HikariDataSource(config);
        log.info("HikariCP pool initialized successfully");
    }

    @Override
    public Connection create() {
        try {
            log.info("Obtaining database connection from HikariCP pool...");
    ...
    @Override
    public void close() {
        log.info("Closing HikariCP data source pool...");
        dataSource.close();
        log.info("HikariCP data source pool closed.");
    }
