...
import io.zabrek.soulbound.logger.filter.LogRecordTypeFilter;
import io.zabrek.soulbound.logger.format.LogfileFormatter;
import io.zabrek.soulbound.logger.handler.LazyHandler;
import io.zabrek.soulbound.logger.handler.ResettableHandler;
import io.zabrek.soulbound.logger.handler.history.BukkitSchedulerCleaningLogQueue;
import io.zabrek.soulbound.logger.handler.history.DiscardingLogQueue;
import io.zabrek.soulbound.logger.handler.history.HistoryHandler;
import io.zabrek.soulbound.logger.handler.history.LogRecordQueue;
...
public final class HandlerFactory {
     ...
     */
    public static HistoryHandler createHistoryHandler(final SoulBoundLoggerFactory loggerFactory, final Plugin plugin, final BukkitScheduler scheduler, final FileConfigAccessor config, final File logFileFolder, final InstantSource instantSource) {
        ...
        historyHandler.setFilter(new LogRecordTypeFilter(SoulBoundLogRecord.class));
        return historyHandler;
    }

    private static ResettableHandler createDebugLogFileHandler(final SoulBoundLoggerFactory loggerFactory, final File logFile, final InstantSource instantSource) {
        final SoulBoundLogger log = loggerFactory.create(LazyHandler.class);
