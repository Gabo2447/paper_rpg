...
/**
 * This is the implementation of the interface {@link SoulBoundLogger}.
 */
@SuppressWarnings("PMD.TooManyMethods")
public class DefaultSoulBoundLogger implements SoulBoundLogger {
     ...
     */
    public DefaultSoulBoundLogger(final Plugin plugin, final Logger parentLogger, final Class<?> clazz, @Nullable final String topic) {
        this.plugin = plugin;
    ...
    public void debug(final LogSource logSource, @Nullable final String msg) {
        final SoulBoundLogRecord record = new SoulBoundLogRecord(Level.FINE, msg, plugin, logSource);
        logger.log(record);
    }
    ...
    public void info(final LogSource logSource, @Nullable final String msg) {
        final SoulBoundLogRecord record = new SoulBoundLogRecord(Level.INFO, msg, plugin, logSource);
        logger.log(record);
    }
    ...
    public void error(final LogSource logSource, @Nullable final String msg) {
        final SoulBoundLogRecord record = new SoulBoundLogRecord(Level.SEVERE, msg, plugin, logSource);
        logger.log(record);
    }
