...
public class MySqlJdbcProvider implements ConnectionProvider {
     ...
     */
    public MySqlJdbcProvider(final SoulBoundLogger log, final String hostname, final String port, final String database, final String user, final String password) {
        this.log = log;
    ...
    public Connection create() {
        ...
        try {
            ...
            final String connClassName = connection.getClass().getName();
            if (!connClassName.startsWith("com.mysql.")) {
                log.warn("External source modified or changed the MySQL connector! We can not guarantee that SoulBound will work correctly with connector '%s'".formatted(connClassName));
            }
