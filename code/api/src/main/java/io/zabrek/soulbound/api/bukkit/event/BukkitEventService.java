...
public interface BukkitEventService {
     ...
     * @throws io.zabrek.soulbound.api.SoulBoundException if the event could not be subscribed to
     * @since 2.0.0
     */
    <T extends Event> EventServiceSubscriber<T> subscribe(Class<T> event, EventPriority priority, boolean ignoreCancelled,
                                                          EventServiceSubscriber<T> subscriber) throws SoulBoundException;
     ...
     * @param <T>        the event type
     * @return the subscriber that was registered
     * @throws SoulBoundException if the event could not be subscribed to
     * @since 2.0.0
     */
    default <T extends Event> EventServiceSubscriber<T> subscribe(final Class<T> event, final EventPriority priority,
                                                                  final EventServiceSubscriber<T> subscriber) throws SoulBoundException {
