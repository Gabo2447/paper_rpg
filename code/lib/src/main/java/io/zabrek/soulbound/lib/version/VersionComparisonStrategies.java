package io.zabrek.soulbound.lib.version;

import io.zabrek.soulbound.api.version.Version;
import io.zabrek.soulbound.api.version.VersionToken;
import io.zabrek.soulbound.api.version.VersionType;
...
public final class VersionComparisonStrategies {
     ...
     * <p>
     * Delegates to {@link #onlyCompare(Set, boolean)} with {@code completeIsEqual} set to {@code true}.
     *
    ...
    public static Comparator<Version> onlyCompare(final Set<String> tokensToCompare) {
        return onlyCompare(tokensToCompare, true);
    }

    /**
     * Exclude the given tokens from comparison.
     * <p>
     * The inverse of {@link #onlyCompare(Set)}.
     *
     * @param tokensToExclude the tokens to exclude from comparison
     * @param completeIsEqual if {@code true} and all given tokens are equal for both compared versions,
     *                        they are considered equal in the context of this strategy
     * @return the comparison strategy that excludes the given tokens from comparison
     */
    public static Comparator<Version> exclude(final Set<String> tokensToExclude, final boolean completeIsEqual) {
        return predicatedStrategy(token -> !tokensToExclude.contains(token.name()), completeIsEqual);
    }
     ...
     * @return the comparison strategy that only compares the given tokens
     */
    public static Comparator<Version> onlyCompare(final Set<String> tokensToCompare, final boolean completeIsEqual) {
        return predicatedStrategy(token -> tokensToCompare.contains(token.name()), completeIsEqual);
    }
    ...
    private static Comparator<Version> predicatedStrategy(final Predicate<VersionToken> tokenToCompare, final boolean completeIsEqual) {
        return (first, second) -> {
            ...
            final Map<Integer, List<VersionToken>> tokenGroups = first.type().tokenGroups();
