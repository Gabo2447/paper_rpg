...
public class VersionTypeBuilder {
    ...
    public DefaultVersionType build() {
        ...
        if (!tokens.getLast().finite()) {
            throw new IllegalStateException("Last token group is not closed. Last token '%s' is not finite.".formatted(tokens.getLast().name()));
        }
    ...
    private VersionTypeBuilder endGroup() {
        ...
        final VersionToken lastToken = tokens.getLast();
